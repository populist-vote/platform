            ...
            "politicians" => {
                for (_name, politician) in yaml.into_iter() {
                    ...
                    let input: db::CreatePoliticianInput =
                        serde_yaml::from_value(politician).unwrap();

                    let result = db::Politician::create_optional(&pool.connection, &input).await;

                    if let Ok(result) = result {
                        match result {
                            Some(politician) => println!(
                                "Created Politician Record: {} {}",
                                politician.first_name, politician.last_name
                            ),
                            None => println!("Politician Record already exists"),
                        }
                    }
                }
            }
