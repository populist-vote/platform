...
-- Create check to ensure that respondent_id is nont null on question_submission if question has allow_anonymous_responses set to false
CREATE OR REPLACE FUNCTION check_respondent_id() RETURNS TRIGGER AS $$
    BEGIN
...
CREATE TRIGGER check_respondent_id_trigger
    BEFORE INSERT OR UPDATE ON question_submission
    FOR EACH ROW EXECUTE PROCEDURE check_respondent_id();
