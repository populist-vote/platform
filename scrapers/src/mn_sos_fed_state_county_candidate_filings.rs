...
pub async fn get_mn_sos_candidate_filings_primary_fed_state_county() -> Result<(), Box<dyn Error>> {
    let pool = db::pool().await;
    let caps = DesiredCapabilities::chrome();
    let driver = WebDriver::new("http://localhost:9515", caps).await?;
    driver.goto("https://candidates.sos.mn.gov").await?;
    ...
    sqlx::query!(
        r#"DROP TABLE IF EXISTS p6t_state_mn.mn_candidate_filings_primary_fed_state_county_2024 CASCADE;"#
    )
    .execute(&pool.connection)
    .await?;
    let create_table_query = format!(
        "CREATE TABLE p6t_state_mn.mn_candidate_filings_primary_fed_state_county_2024 (
        {}
    );",
    ...
    sqlx::query(&create_table_query)
        ...
        .await?;
    let mut tx = pool.connection.acquire().await?;
    let copy_query = r#"COPY p6t_state_mn.mn_candidate_filings_primary_fed_state_county_2024 FROM STDIN WITH CSV HEADER;"#;
    let mut tx_copy = tx.copy_in_raw(copy_query).await?;
    tx_copy.send(csv_data_as_string.as_bytes()).await?;
    tx_copy.finish().await?;
