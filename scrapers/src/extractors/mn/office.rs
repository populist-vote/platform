...
pub fn extract_office_name(input: &str) -> Option<String> {
    ...
    let judicial_regex = JUDICIAL_DISTRICT_REGEX
        .get_or_init(|| Regex::new(r"(Judge - [0-9]{1,3}(st|nd|rd|th)? District)").unwrap());
...
pub fn extract_office_district_type(
    input: &str,
    ...
    match election_scope {
        ...
        ElectionScope::District => {
            match district_type {
                ...
                Some(DistrictType::StateSenate) | Some(DistrictType::StateHouse) => {
                    PoliticalScope::State
                }
...
pub fn extract_office_election_scope(
    input: &str,
    county_id: Option<i32>,
) -> Option<db::ElectionScope> {
    let input_lower = input.to_lowercase();
...
pub fn extract_hospital_district(input: &str) -> Option<String> {
    let paren_regex =
        HOSPITAL_DISTRICT_PAREN_REGEX.get_or_init(|| Regex::new(r"\(([^)]+)\)").unwrap());

    // Check for "Hospital District Board Member ([0-9]{1,2})" pattern
    // If matched, extract content from parentheses (not the number)
    let numbered_regex = HOSPITAL_DISTRICT_NUMBERED_REGEX
        .get_or_init(|| Regex::new(r"Hospital District Board Member [0-9]{1,2}").unwrap());
...
pub fn extract_municipality(
    input: &str,
    _election_scope: &ElectionScope,
    _district_type: &Option<DistrictType>,
) -> Option<String> {
    let input_lower = input.to_lowercase();

    // City Offices - extract municipality from parentheses
    let should_extract = input_lower.contains("city clerk - treasurer") ||
        input_lower.contains("city clerk") ||
        input_lower.contains("city treasurer") ||
        input_lower.contains("council member") ||
        input_lower.contains("mayor") ||
...
mod tests {
    ...
    fn extract_office_attributes() {
        ...
        for (
            ...
            (
                ...
                expected_title,
                expected_chamber,
                expected_district_type,
                expected_political_scope,
                expected_election_scope,
            ),
        ) in tests
        {
            ...
            assert_eq!(
                chamber, expected_chamber,
                "Failed to extract chamber from: {}",
                input
            );
            assert_eq!(
                district_type, expected_district_type,
                "Failed to extract district type from: {}",
                input
            );
            assert_eq!(
                political_scope, expected_political_scope,
                "Failed to extract political scope from: {}",
                input
            );
            assert_eq!(
                election_scope, expected_election_scope,
                "Failed to extract election scope from: {}",
                input
            );
        }
    }
    ...
    fn extract_district_type() {
        let tests = vec![
            ...
            (
                "U.S. Representative",
                None,
                Some(db::DistrictType::UsCongressional),
            ),
            (
            ...
            (
                ...
                Some(2),
                Some(db::DistrictType::SoilAndWater),
            ),
            (
                "Soil and Water Supervisor",
                Some(10),
            ...
            (
                ...
                None,
                Some(db::DistrictType::Hospital),
            ),
            // Judicial
            (
                "District Court Judge",
                None,
                Some(db::DistrictType::Judicial),
            ),
            // Non-matching cases
    ...
    fn extract_election_scope() {
        let tests = vec![
            ...
            (
                "Hospital District Board Member at Large",
                None,
                Some(db::ElectionScope::District),
            ),
            (
                "Hospital District Board Member (Cook County)",
                None,
                Some(db::ElectionScope::District),
            ),
            (
