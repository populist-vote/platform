...
/// Normalize contest name: remove "(Vote for 1)" from the end (case-insensitive) and "DEM " or "REP " from the front.
fn normalize_contest_name(s: &str) -> String {
    let mut s = s.trim();
    // Remove "REP " or "DEM " from the front
    s = if s.starts_with("REP ") {
        s[4..].trim_start()
    } else if s.starts_with("DEM ") {
        s[4..].trim_start()
    } else {
        s
    };
    // Remove "(Vote for 1)" from the end (case-insensitive)
    let suffix = "(Vote for 1)";
    s = if s.len() >= suffix.len() && s[s.len() - suffix.len()..].eq_ignore_ascii_case(suffix) {
        s[..s.len() - suffix.len()].trim_end()
    } else {
        s
    };
    s.to_string()
...
fn parse_contest_name_for_office(rule: &CountyOfficeRule, contest_name: &str) -> ParsedContestName {
    let district = extract_office_district(contest_name, None);
    ...
    match office_name {
        ...
        "Judge - Probate Court" => {
            ...
            let name = if county_lower == "collin" {
                "Probate Court"
            } else if county_lower == "galveston" || county_lower == "denton" {
...
pub fn parse_clarity_csv(
    ...
    for result in rdr.deserialize() {
        ...
        rows.push(StgTxClarityResultRow {
            ...
            votes_for_candidate: raw.ballots_cast,
