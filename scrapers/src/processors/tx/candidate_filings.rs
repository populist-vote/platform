...
fn process_tx_office(filing: &TxCandidateFiling) -> Result<Office, Box<dyn Error>> {
    ...
    let (political_scope, election_scope, district_type) =
        office::extract_office_scope(&name, county.as_deref(), seat.as_deref())
...
async fn apply_same_person_updates_and_dupes(
    ...
    } else if existing_email_trimmed.is_none()
        && politician
            .email
            .as_deref()
            .map(|e| e.trim())
            ...
            .is_some()
    {
        let incoming_trimmed = politician
            .email
            .as_deref()
            .map(|e| e.trim())
            .filter(|e| !e.is_empty())
