...
WITH matches AS (
    SELECT 
        ...
        stg.full_name AS staging_full_name,
        prod.full_name AS production_full_name,
        stg.home_state AS staging_state,
        prod.home_state AS prod_state,
        ...
        CASE WHEN stg.email = prod.email AND stg.email IS NOT NULL AND prod.email IS NOT NULL AND stg.email <> '' AND prod.email <> '' THEN 'exact_email'
             ...
             WHEN stg.residence_address_id IS NOT NULL AND prod.residence_address_id IS NOT NULL
                  AND stg_addr.id IS NOT NULL AND prod_addr.id IS NOT NULL
                  AND TRIM(LOWER(stg_addr.line_1)) = TRIM(LOWER(prod_addr.line_1))
                  AND TRIM(LOWER(stg_addr.city)) = TRIM(LOWER(prod_addr.city))
                  AND TRIM(stg_addr.state) = TRIM(prod_addr.state::text)
                  AND TRIM(LOWER(stg_addr.country)) = TRIM(LOWER(prod_addr.country))
             THEN 'exact_address'
    ...
    FROM ingest_staging.stg_tx_politicians stg
    CROSS JOIN politician prod
    LEFT JOIN ingest_staging.stg_tx_addresses stg_addr ON stg.residence_address_id = stg_addr.id
    LEFT JOIN address prod_addr ON prod.residence_address_id = prod_addr.id
    WHERE prod.home_state = 'TX'
    AND (
        ...
        OR (stg.residence_address_id IS NOT NULL AND prod.residence_address_id IS NOT NULL
            AND stg_addr.id IS NOT NULL AND prod_addr.id IS NOT NULL
            AND TRIM(LOWER(stg_addr.line_1)) = TRIM(LOWER(prod_addr.line_1))
            AND TRIM(LOWER(stg_addr.city)) = TRIM(LOWER(prod_addr.city))
            AND TRIM(stg_addr.state) = TRIM(prod_addr.state::text)
...
ORDER BY 
    CASE match_type
        ...
        WHEN 'exact_address' THEN 3
        WHEN 'exact_name' THEN 4
        WHEN 'exact_slug' THEN 5
...
JOIN politician prod ON prod.home_state = 'TX'
    AND LOWER(stg.first_name) = LOWER(prod.first_name)
    AND LOWER(stg.last_name) = LOWER(prod.last_name)
    AND (
        (COALESCE(TRIM(stg.middle_name), '') = '' AND COALESCE(TRIM(prod.middle_name), '') = '')
        OR (COALESCE(TRIM(stg.middle_name), '') <> '' AND COALESCE(TRIM(prod.middle_name), '') <> '' AND LOWER(TRIM(stg.middle_name)) = LOWER(TRIM(prod.middle_name)))
