...
impl BallotMeasureMutation {
    ...
    async fn update_ballot_measure(
        ...
        let db_pool = ctx.data_unchecked::<Pool<Postgres>>();
        let updated_record = BallotMeasure::update(db_pool, uuid::Uuid::parse_str(&id)?, &input).await?;
        Ok(BallotMeasureResult::from(updated_record))
    }

    async fn delete_ballot_measure(&self, ctx: &Context<'_>, id: String) -> Result<DeleteBallotMeasureResult> {
        let db_pool = ctx.data_unchecked::<Pool<Postgres>>();
