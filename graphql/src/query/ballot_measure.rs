...
impl BallotMeasureQuery {
    async fn all_ballot_measures(&self, ctx: &Context<'_>) -> FieldResult<Vec<BallotMeasureResult>> {
        let pool = ctx.data_unchecked::<Pool<Postgres>>();
        let records = BallotMeasure::index(pool).await?;
        let results = records.into_iter().map(|r| BallotMeasureResult::from(r)).collect();
        Ok(results)
    }
